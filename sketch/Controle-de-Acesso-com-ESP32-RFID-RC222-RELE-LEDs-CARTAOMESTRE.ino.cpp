...
void setup()
{
  ...
  {                                    // Quando o botão for pressionado, o pino deve ficar baixo, o botão está conectado ao GND
    ...
    if (buttonState == true && digitalRead(BotaoWipe) == LOW)
    { // Se o botão ainda estiver pressionado, apaga a EEPROM.
      Serial.println(F("Inicio da formatacao da EEPROM"));
      for (uint16_t x = 0; x < EEPROM.length(); x = x + 1)
      { // Fim do loop do endereço da EEPROM
        if (EEPROM.read(x) == 0)
        { // Se endereço da EEPROM for 0
  ...
  {                                     // Ler o UID do Cartão Mestre na EEPROM.
...
void loop()
{
  ...
  do
  {
    ...
    { // Verifique se o botão está pressionado
      ...
      if (buttonState == true && digitalRead(BotaoWipe) == LOW)
      {                     // Se o botão ainda estiver pressionado, limpe a EEPROM.
